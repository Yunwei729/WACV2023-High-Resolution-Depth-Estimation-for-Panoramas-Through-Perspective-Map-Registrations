   ...
   int CheckGLError(char *file, int line)
   {
	   ...
	   while (glErr != GL_NO_ERROR) 
       {
		 ...
		 if (sError)
		    ...
		    std::cout << "GL Error #" << glErr << " (no message available)" << " in File " << file << " at line: " << line << endl;
...
//----------------------------------------------------------------------------- 
   ...
   bool HasGLSLSupport(void)
   {
      ...
      if (GLEW_VERSION_2_0)
      {
         std::cout << "OpenGL 2.0 (or higher) is available!" << endl;
      }
      else if (GLEW_VERSION_1_5)
      {
         std::cout << "OpenGL 1.5 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_4)
      {
         std::cout << "OpenGL 1.4 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_3)
      {
         std::cout << "OpenGL 1.3 core functions are available" << endl;
      }
      else if (GLEW_VERSION_1_2)
      {
         std::cout << "OpenGL 1.2 core functions are available" << endl;
      }

      if (GL_TRUE != glewGetExtension("GL_ARB_fragment_shader"))
      {
          std::cout << "[WARNING] GL_ARB_fragment_shader extension is not available!\n";
          useGLSL = false;
      }

      if (GL_TRUE != glewGetExtension("GL_ARB_vertex_shader"))
      {
          std::cout << "[WARNING] GL_ARB_vertex_shader extension is not available!\n";
          useGLSL = false;
      }

      if (GL_TRUE != glewGetExtension("GL_ARB_shader_objects"))
      {
          std::cout << "[WARNING] GL_ARB_shader_objects extension is not available!\n";
          useGLSL = false;
      }
      ...
      {
          std::cout << "[FAILED] OpenGL Shading Language is not available...\n\n";
      }   
...
glShader::glShader()
{
  ...
  if (!useGLSL)
  {
     std::cout << "**ERROR: OpenGL Shading Language is NOT available!" << endl;
  }
  else
  {
...
if (!useGLSL) return;
   ...
   {
        std::cout << "**warning** please compile program before adding object! trying to compile now...\n";
...
unsigned int i;
    ...
    {
       std::cout << "**warning** Object is already linked, trying to link again" << endl;
...
glShader* glShaderManager::loadfromFile(char* vertexFile, char* fragmentFile) 
{
   ...
   { 
     std::cout << "error: can't load vertex shader!\n"; 
     delete o;
  ...
  {
     std::cout << "error: can't load fragment shader!\n";
     delete o;
     delete tVertexShader;
     delete tFragmentShader;
     return 0;
  ...
  if (!tVertexShader->compile())
  {
      ...
      std::cout << tVertexShader->getCompilerLog() << endl;
      delete o;
      delete tVertexShader;
      delete tFragmentShader;
      return 0;
  }
  std::cout << "***GLSL Compiler Log (Vertex Shader):\n";
  std::cout << tVertexShader->getCompilerLog() << "\n";
  ...
  if (!tFragmentShader->compile())
  {
     ...
     std::cout << tFragmentShader->getCompilerLog() << endl;
  ...
  std::cout << "***GLSL Compiler Log (Fragment Shader):\n";
  std::cout << tFragmentShader->getCompilerLog() << "\n";
  ...
  {
     std::cout << "**LINKER ERROR\n";
     std::cout << o->getLinkerLog() << endl;
     delete o;
     delete tVertexShader;
     delete tFragmentShader;
     return 0;
  }
  std::cout << "***GLSL Linker Log:\n";
  std::cout << o->getLinkerLog() << endl;
...
glShader* glShaderManager::loadfromFile(char* vertexFile, char* geometryFile, char* fragmentFile)
{
  ...
  if (geometryFile!=0)
     ...
     {
        std::cout << "error: can't load geometry shader!\n";
        delete o;
        delete tVertexShader;
        delete tFragmentShader;
        delete tGeometryShader;
  ...
  if (geometryFile!=0)
  {
     ...
     {
        std::cout << "***COMPILER ERROR (Geometry Shader):\n";
        std::cout << tGeometryShader->getCompilerLog() << endl;
  ...
  std::cout << "***GLSL Compiler Log (Geometry Shader):\n";
  std::cout << tGeometryShader->getCompilerLog() << "\n";
